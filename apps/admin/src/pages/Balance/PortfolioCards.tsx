...
export default function PortfolioCards({ portfolios }: PortfolioCardsProps) {
  return (
    <div className="grid auto-rows-fr gap-4 grid-cols-[repeat(auto-fit,minmax(240px,1fr))]">
      {portfolios.map(p => (
        <PortfolioCard key={p.id} stockName={p.stockName} volume={p.volume} />
      ))}
    </div>
  )
}
...
function PortfolioCard({ stockName, volume = 0 }: PortfolioCardProps) {
  return (
...
function copyVolume(volume: NonNullable<MasterPortfolioWithId["volume"]>) {
